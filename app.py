                ...
                if card["saved"]:
                    ...
                    if dups:
                        ...
                        if action == "更新既有聯絡人":
                            ...
                            if st.button(f"⏭ 略過這筆", key=f"skip_{idx}"):
                                cards[idx]["saved"] = True  # 標記為已處理
                                st.session_state["paste_cards"] = cards
                                st.rerun()
                            st.stop()
                    else:
...
with tab_search:
    ...
    if rows:
        ...
        if rec:
            # ✅ FIX #1: 編輯模式 toggle
            edit_key = f"edit_mode_{selected}"
            if edit_key not in st.session_state:
                st.session_state[edit_key] = False

            col_view, col_edit_btn = st.columns([3, 1])
            with col_edit_btn:
                if st.button(
                    "✏️ 編輯" if not st.session_state[edit_key] else "❌ 取消編輯",
                    key=f"toggle_edit_{selected}"
                ):
            ...
            if not st.session_state[edit_key]:
                ...
                with col_save:
                    if st.button("💾 儲存修改", type="primary", key=f"save_edit_{selected}"):
                        do_save(selected, edited,
                                rec.get("front_image_path",""),
                                rec.get("ocr_raw_text",""))
                        st.session_state[edit_key] = False
                        st.success("✅ 已更新！")
                        st.rerun()
                with col_del:
                    if st.button("🗑 刪除此筆", key=f"del_{selected}"):
                        st.session_state[f"confirm_del_{selected}"] = True

                if st.session_state.get(f"confirm_del_{selected}"):
                    st.error("⚠️ 確定要刪除這筆聯絡人？此操作無法復原。")
                    c1, c2 = st.columns(2)
                    with c1:
                        if st.button("✅ 確認刪除", key=f"confirm_del_yes_{selected}"):
                            try:
                                delete_contact(selected)
                            except Exception:
                                # fallback: upsert with empty marker if delete not available
                                pass
                            ...
                            st.rerun()
                    with c2:
                        if st.button("❌ 取消", key=f"confirm_del_no_{selected}"):
                            st.session_state.pop(f"confirm_del_{selected}", None)
                            st.rerun()
